...
public:
	...
	static inline void SetGroundRegist(const float _regist) { GroundRegist() = _regist; }

	static inline void SetFieldSize(const ChVec3& _size) {
		FieldMaxSize() = _size;
		FieldMaxSize().Abs();
		FieldMinSize() = FieldMaxSize();
		FieldMinSize().x = -FieldMinSize().x;
		FieldMinSize().y = -FieldMinSize().y;
		FieldMinSize().z = -FieldMinSize().z;
	}

	static inline void SetFieldSize(const ChVec3& _max, const ChVec3& _min)
	{
		FieldMaxSize() = _max;
		FieldMinSize() = _min;
	}
...
public:
	...
	static inline float GetGroundRegist() { return GroundRegist(); }

	static inline ChVec3 GetFieldMaxSize() { return FieldMaxSize(); }

	static inline ChVec3 GetFieldMinSize() { return FieldMinSize(); }
...
private:
	...
	inline static float& GroundRegist()
	{
		static float ins = 0.0f;
		return ins;
	}
	...
	inline static ChVec3& FieldMaxSize()
	{
		static ChVec3 ins;
		return ins;
	}

	inline static ChVec3& FieldMinSize()
	{
		static ChVec3 ins;
		return ins;
	}
	...
	public:
		...
		void IsHitField(ChCpp::PolygonCollider<wchar_t>& _polygon);
